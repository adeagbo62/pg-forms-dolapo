...
$sql = "SELECT s.id, s.studentName, s.matricNo 
        FROM students s 
        LEFT JOIN dean_endorsements e ON s.matricNo = e.matricNo 
        WHERE s.sub_dean_comment IS NOT NULL AND e.matricNo IS NULL";
...
<head>
    ...
    <title>College Dean - Homepage</title>
</head>
<body>
    <div class="header-1">
        <img src="image/covenant-university-logo-desktop.png" alt="">
        <h1>Consent and Quality Attestation Form (FORM N) - Dean (SPS)</h1>                
    </div>
    <section>
        <h1>Welcome, <span id="hod"><i>Dr. Pope John Paul</i></span></h1>
    ...
    const boxes = document.querySelectorAll(".endorse-btn");
        boxes.forEach((box)=>{
            box.addEventListener("click", function(){
                ...
                window.location.href = `deanPage.php?id=${studentId}`;
            });
        })
